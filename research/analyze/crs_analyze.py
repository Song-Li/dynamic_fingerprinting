...
class Feature_Lists(Enum):
    All = "fontlist, inc, gpu, timezone, plugins, cookie, localstorage, gpuimgs, adblock, cpu_cores, canvas_test, audio, langsdetected, agent, accept, encoding, language, jsFonts, WebGL, flashFonts, cc_audio, hybrid_audio".replace(" ", "").split(',')
...
def get_lang_entropy(cursor):
    cursor.execute("SELECT langsdetected from {}".format(table_name))
    lang_to_boxes = {}
...
def index():
    LaTex = True
    db = MySQLdb.connect("localhost", "root", "Mong!#%8", db_name)
    cursor = db.cursor()
    ...
    table = []
    #get_lang_entropy(cursor)
    for feat in Feature_Lists.All:
        print feat
        table += get_feature_entropy(cursor, feat)
    table += get_feature_entropy(cursor, ','.join(Feature_Lists.All))
    print_table(table)
